...
final class ScaffoldSafewalkPosition extends Module
{
    ...
    public void enable()
    {
        applyingConsumer = (user, event) -> {
            ...
            if (!user.hasMovedRecently(TimeKey.LAST_XZ_MOVEMENT, 175) ||
                ...
                user.getPlayer().isSneaking() && user.getTimestampMap().at(TimeKey.LAST_SNEAK_ENABLE).passedTime() > MIN_SNEAK_BYPASS_MILLIS) return 0;
            ...
            if (sneakBorder) {
                if (user.getDataMap().getCounter(DataKey.Count.SCAFFOLD_SAFEWALK_POSITION_FAILS).incrementCompareThreshold()) {
                    DebugSender.INSTANCE.sendDebug("Scaffold-Debug | Player: " + user.getPlayer().getName() + " has behaviour associated with safe-walk. (Position)" + " | Face: " + face + "| xOffset: " + xOffset + " | zOffset" + zOffset);
                    return 15;
                }
