...
public class ModuleLoader
{
    ...
    public boolean load()
    {
        if (this.bungeecordForbidden && AACAdditionPro.getInstance().isBungeecord()) {
            DebugSender.getInstance().sendVerboseMessage(module.getConfigString() + " is not compatible with bungeecord.", true, false);
            return false;
        }
        ...
        if (!ServerVersion.supportsActiveServerVersion(allowedServerVersions)) {
            DebugSender.getInstance().sendVerboseMessage(module.getConfigString() + " is not compatible with your server version.", true, false);
            return false;
        }
