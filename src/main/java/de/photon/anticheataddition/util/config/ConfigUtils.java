...
public final class ConfigUtils
{
    ...
    public static void processLoadFromConfiguration(final Object object, String prePath)
    {
        ...
        for (Field field : object.getClass().getDeclaredFields()) {
            ...
            if (!ServerVersion.is18()) Preconditions.checkArgument(AntiCheatAddition.getInstance().getConfig().contains(path, true), "Path " + path + " does not exist.");

            // The different classes
            try {
                // Boolean
                if (type == boolean.class || type == Boolean.class) {
                    field.setBoolean(object, AntiCheatAddition.getInstance().getConfig().getBoolean(path));
                }
                // Numbers
                else if (type == double.class || type == Double.class) {
                    field.setDouble(object, AntiCheatAddition.getInstance().getConfig().getDouble(path));
                } else if (type == int.class || type == Integer.class) {
                    field.setInt(object, AntiCheatAddition.getInstance().getConfig().getInt(path));
                } else if (type == long.class || type == Long.class) {
                    field.setLong(object, AntiCheatAddition.getInstance().getConfig().getLong(path));
                } else if (type == String.class) {
                    field.set(object, AntiCheatAddition.getInstance().getConfig().getString(path));
                }
                // Special stuff
                else if (type == ItemStack.class) {
                    field.set(object, AntiCheatAddition.getInstance().getConfig().getItemStack(path));
                } else if (type == Color.class) {
                    field.set(object, AntiCheatAddition.getInstance().getConfig().getColor(path));
                } else if (type == OfflinePlayer.class) {
                    field.set(object, AntiCheatAddition.getInstance().getConfig().getOfflinePlayer(path));
                } else if (type == Vector.class) {
                    field.set(object, AntiCheatAddition.getInstance().getConfig().getVector(path));
                }
                // Lists
                    ...
                    if (annotation.listType() == String.class) {
                        ...
                        field.set(object, AntiCheatAddition.getInstance().getConfig().getList(path));
                    }
                }
                // No special type found
                else {
                    field.set(object, AntiCheatAddition.getInstance().getConfig().get(path));
                }
            } catch (IllegalAccessException e) {
                AntiCheatAddition.getInstance().getLogger().log(Level.SEVERE, "Unable to load config value due to unknown type.", e);
            }
        }
    }
